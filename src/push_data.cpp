...
bool Push_data::thingspeak_push_raw(const int *values,
									int index_start,
									int values_n,
									char *buffer)
{
	WiFiClientSecure client;
	if (client.connect(CONFIG.push.host, 443) == 0) {
		LOG_WARN("Cannot connect '%s'", CONFIG.push.host);
		return false;
	}
	LOG_INFO("Push data to '%s' index %d - %d",
			 CONFIG.push.host,
			 index_start,
			 index_start + values_n);

	// cbuffer "https://api.thingspeak.com/update?api_key=<api key>&field1=0&field2=12"
	int buffer_offset = snprintf(buffer,
								 buffer_size,
	...
	for (int loop = 0; loop < values_n; loop++) {
		ret = snprintf(buffer + buffer_offset,
...
bool Push_data::thingspeak_push(const int *values,
								int index_start,
								int values_n,
								bool force_update)
{
	if (!force_update && !timer.check(push_interval_s * 1000))
		return false;
