...
bool Logic::check_timer_safety_ok(int pump, int water_level, int water_switch)
{
	...
	if (!pump_start_check_done) {
		...
		if (water_level <= 0) {
			LOG_FATAL("Water level too low (%d), pump not started.",
					  water_level);
			return false;
		}
		...
		if (pump_timer.check(CONFIG.pump.threshold_water_up_s * 1000)) {
			if (water_switch != 1) {
				LOG_FATAL(
					"Water switch not active, even pump running for the Time.");
				return false;
			}
		}
		return true;
...
LogicStatus Logic::calculate_status(int pump_value, int water_switch)
{
	...
	const LogicStatus states[] =
		{LogicStatus::idle, LogicStatus::pump_started, LogicStatus::draining,
		 LogicStatus::pumping};
	return states[pump_value + (water_switch << 1)];
}

bool Logic::run_logic(const Config_run_table_time *time_now,
