...
Water_level::Water_level(const char *name,
						 uint8_t pin_trigger,
						 uint8_t pin_echo)
	:
	Device_input(name)
...
void Water_level::measure_pulse_in()
{
	...
	if (this->value == 0) {  // in the beginning, boost directly to proper level
		ma_old = distance;
	}
...
void Water_level::loop()
{
	if (nfails >= nfail_limit) {
		if (nfails == nfail_limit) {  // log the error only once (to avoid spamming..)
